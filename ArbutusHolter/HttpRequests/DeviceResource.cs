...
namespace Uvic_Ecg_ArbutusHolter.HttpRequests
{
    class DeviceResource
    {
        ...
        public RestModel<Device> GetAvailableDevices(Client client, DateTime pickup, DateTime returnT)
        {
            restModel = requests.GetAll("test/devices?pickupDate=" + pickup.ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss")
                                        + "-0000&deviceReturnDate=" + returnT.ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss") + "-0000&deviceLocation=Victoria&clinicId=1", client);
            return restModel;
        }
